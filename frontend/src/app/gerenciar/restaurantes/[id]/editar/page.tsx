...
import { getRestaurante, updateRestaurante } from '@/services/restaurantes';
...
export default function EditarRestaurantePage() {
  ...
  useEffect(() => { if (imagemValue !== undefined) setPreviewImg(imagemValue || ''); }, [imagemValue]);
    ...
    getRestaurante(id)
      .then((r) => {
        reset({
          ...
          imagem: r.imagem ?? '',
          endereco: r.endereco ?? '',
          tempo_entrega: r.tempo_entrega ?? '30-45 min',
          pedido_minimo: r.pedido_minimo ?? 0,
          aberto: r.aberto ?? true,
        });
        setPreviewImg(r.imagem ?? '');
      })
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      setAlert({ type: 'success', message: 'Restaurante atualizado!' });
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            <div className="space-y-3">
              <Input label="Nome do restaurante *" {...register('nome')} error={errors.nome?.message} />
